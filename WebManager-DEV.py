...
def atualizar_tabela_com_dados():
    global df
    df = carregar_dados()  
    atualizar_tabela()  
...
def salvar_dados_edit(df):
    ...
    if not all([entry_nome.get(), entry_url.get(), entry_padronizado.get()]):
    ...
    def confirmar_edicao():
        ...
        salvar_dados_edit.janela_edicao_aberta = False
...
ctk.CTkButton(frame_botoes, fg_color="#8093F1", hover_color="#5A70ED", text="Atualizar tabela", command=lambda: atualizar_tabela_com_dados()).grid(row=3, column=0, pady=10)
